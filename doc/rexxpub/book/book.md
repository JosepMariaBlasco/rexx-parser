...
- **rexxdoc-chapter** replicates the visual style of the official
  ooRexx documentation (generated by DocBook XSL Stylesheets with
  Apache FOP).  It uses Liberation Sans typography, blue headings,
  DocBook-style paragraph spacing, and an automatic "Chapter N."
  label.  It is designed for individual chapters of the ooRexx
...
For documents that don't follow the filename convention, the document
class can be specified in the YAML front matter with `docclass:`.
For example, to write an ooRexx documentation chapter in a file
called `keyword-instructions.md`:
...
`slides`; `book.md` uses `book`; `letter.md` uses `letter`;
`rexxdoc-chapter.md` uses `rexxdoc-chapter`.  Any other filename is
first tried as a document class name (which requires a corresponding
CSS file in the `css/print` directory); if no such file exists, the
`default` class is used as a fallback.

The document class can also be specified in the YAML front matter,
...
This allows any file, regardless of its name, to use a specific
document class.  For example, a file called `my-paper.md` with
`docclass: article` in its YAML will be rendered with the `article`
class without needing a command-line override.

For cases where neither the filename nor the YAML specify the desired
class, the `--docclass` command-line option provides an explicit
override:
...
The precedence order is: YAML front matter, then command-line option,
then filename convention, then `default` as a fallback.
...
The `rexxdoc-chapter` Class {.chapter}
===========================
...
individual chapters rendered online via the CGI pipeline, and PDF
generation of chapters or standalone technical documents via md2pdf.

The reference document is `rexxdoc-chapter.md`, which is itself
rendered by the class it describes.
...
can be turned into a rexxdoc chapter by adding a few lines of YAML
...
manuals: Liberation Sans at 10pt for body text, Liberation Mono at
8pt for code blocks, left-aligned paragraphs with no justification
or hyphenation, and paragraphs separated by vertical space (DocBook
style) with no first-line indent.

Headings are set in Liberation Sans Bold, in blue (`#336699`).
Links use a slightly different blue (`#0066cc`), with no underline.

Heading hierarchy
-----------------

The `rexxdoc-chapter` class shifts the heading hierarchy by one level
compared to other RexxPub classes.  `<h1>` is the chapter title,
with a "Chapter N." label generated automatically via CSS `::before`
and a thin horizontal rule below the label.  `<h2>` headings are
sections, numbered N.1, N.2, etc.  `<h3>` headings are subsections,
numbered N.1.1, etc.  `<h4>` headings are sub-subsections, numbered
N.1.1.1, etc.
...
If the file is named `rexxdoc-chapter.md`, the `docclass` can be
omitted (it will be inferred from the filename).  The `chapter`
value defaults to 1 if not specified.  It determines both the
"Chapter N." label above the title and the prefix for all section
numbers (7.1, 7.2, 7.1.1, etc.).
...
In the print rendering (via `?print=pdf` or md2pdf), the page layout
replicates the official ooRexx documentation: the current section
title appears as a running header at the top right of each page,
separated from the content by a thin horizontal rule.  Page numbers
are centred at the bottom, with a thin rule above.  Margins are
asymmetric, matching the FOP output --- approximately 31mm on the
left (binding side) and 19mm on the right.

Note and warning boxes
----------------------

The ooRexx documentation uses coloured boxes for notes and warnings.
The `rexxdoc-chapter` class supports these through Pandoc fenced
divs:
...
Note boxes use an olive header (`#8e9e00`); warning boxes use a red
header.  The header bar is automatically generated by CSS --- the
Markdown source only needs the fenced div wrapper.

Relationship with other classes
-------------------------------

The `rexxdoc-chapter` class is loaded after `rexxpub-base.css`, just
like the other paginated classes.  It inherits all the shared
infrastructure: section numbering, figure and listing captions,
footnote support, and paged.js split-element fixes.

The key differences from the LaTeX-inspired classes (`article`,
`book`) are: sans-serif typography instead of Times New Roman,
DocBook-style paragraph spacing instead of first-line indent, blue
headings, an automatic chapter label with horizontal rule, section
numbers prefixed with the chapter number, and asymmetric page
margins.

A companion `rexxdoc` class for full multi-chapter books is planned
...
book; `rexxdoc-chapter.md` is an ooRexx documentation chapter.  Any
other name (such as `readme.md` or `installation.md`) uses the
`default` class in md2pdf, and the web-only `markdown` stylesheet in
the CGI program.
...
filename is the specification.

However, when the filename convention is not convenient --- for
example, when writing an ooRexx documentation chapter in a file
called `keyword-instructions.md` --- the document class can be
...
The YAML `docclass` takes precedence over both the filename
convention and the `--docclass` command-line option.  This ensures
that the author's intent, encoded in the document itself, is always
respected.
...
`chapter` (chapter number, used by `rexxdoc-chapter`),
...
The `docclass` option is particularly useful for the
`rexxdoc-chapter` class, where the filename convention is rarely
practical --- ooRexx documentation chapters have descriptive names
like `keyword-instructions.md`, not `rexxdoc-chapter.md`.  The
`chapter` option sets the chapter number that appears in the
"Chapter N." label and as a prefix for all section numbers.
...
`rexxdoc-chapter.css` is the ooRexx documentation chapter class (no
size variants).
